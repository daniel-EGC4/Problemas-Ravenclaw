...
const procesarBibliotecaPremium = (recursos) => {
    const filtrar = recursos.filter((p) => p.esPremium === true);
    const ordenar = filtrar.sort ((a, b) => b.calificacion - a.calificacion );
    const rehacer = ordenar.map(({ titulo, creador: autor, calificacion }) => {
      return { titulo, calificacion, autor };
    });
    return rehacer
};

console.log(procesarBibliotecaPremium(recursos))
